...
var dropzoneInit = function dropzoneInit() {
  ...
  !!dropzones.length && dropzones.forEach(function (item) {
    ...
    var options = merge({
      ...
      //maxFiles: 20,     //파일 최대업로드수
      maxFilesize: 100,   //파일사이즈 100메가 까지
...
var tooltipInit = function tooltipInit() {
  ...
  // 툴팁과 드롭다운을 동시 사용을 못해서 툴팁(data-bs-toggle2) 태그를 하나 더 만듬.
...
// 전체영역의 툴팁 초기화가 아닌 div 영역안에서 초기화 할경우 (동적으로 html 태그를 생성했을경우)
var tooltipArea = (div)=>{
  var tooltipTriggerList = [].slice.call(document.querySelectorAll(`${div} [data-bs-toggle="tooltip"]`));
  tooltipTriggerList.map(function (tooltipTriggerEl) {
      return new window.bootstrap.Tooltip(tooltipTriggerEl, {
          trigger: 'hover'
      });
  ...
  var tooltipTriggerList = [].slice.call(document.querySelectorAll(`${div} [data-bs-toggle2="tooltip"]`));
  tooltipTriggerList.map(function (tooltipTriggerEl) {
    return new window.bootstrap.Tooltip(tooltipTriggerEl, {
      trigger: 'hover'
    });
